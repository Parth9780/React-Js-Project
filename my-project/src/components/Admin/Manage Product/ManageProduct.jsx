...
export const ManageProduct = () => {
    ...
    useEffect(() => {
        axios.get(`http://localhost:8000/product`).then((responce) => {
                                            ...
                                            return (
                                                <>
                                                    <tr className='text-center'>
                                                        ...
                                                        <td className='px-3 lg:px-5 lg:py-5' style={tableStyle}>{row.categoryname}</td>
                                                        <td className='px-3 lg:px-5 lg:py-5' style={tableStyle}>{row.subcategoryname}</td>
                                                        <td className='px-3 lg:px-5 lg:py-5' style={tableStyle}>{row.productname}</td>
                                                        <td className='px-3 lg:px-5 lg:py-5' style={tableStyle}>{row.offerprice}</td>
                                                        <td className='px-3 lg:px-5 lg:py-5' style={tableStyle}>{row.qty}</td>
                                                        <td className='px-3 lg:px-5 lg:py-5' style={tableStyle}>{row.added_date}</td>
                                                        <td className='px-3 lg:px-5 lg:py-5' style={tableStyle}><button onClick={() => { navigate(`/admin/dashboard/delete-product/${row.id}`) }}><i class="bi bi-trash"></i></button></td>
                                                    </tr>
                                                </>
                                            )
                                        })}
